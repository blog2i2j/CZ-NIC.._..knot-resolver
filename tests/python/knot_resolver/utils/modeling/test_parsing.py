...
from knot_resolver.utils.modeling.parsing import parse_json_file, parse_yaml_file, try_to_parse_file
...
def test_parse_yaml_duplicity(file: str) -> None:
    file_path = base_path / file
    with pytest.raises(DataParsingError):
...
def test_try_to_parse_file_include_root(file: str) -> None:
    file_path = base_path / file
    wrapped_data = try_to_parse_file(file_path)
    assert wrapped_data.file == file_path
...
def test_try_to_parse_file_include_inner(file: str) -> None:
    file_path = base_path / file
    wrapped_data = try_to_parse_file(file_path)
    assert wrapped_data.file == file_path
